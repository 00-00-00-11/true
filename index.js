...
discord.on("ready",  () => {
    ...
    discord.user.setPresence({
        game:{
            name: "in CYBER-SKY with CYBERS", url: "https://www.twitch.tv/eklavya011"
...
discord.on('guildMemberAdd', member => {
    ...
    const channel = member.guild.channels.find(ch => ch.id === '502323439890071553'); 
...
if(message.content.toLowerCase() == "hi"){
    message.reply("oh,hi dude. how is it going,hope its good ")
}
...
if (util.commandIs(message, "tot")){
    message.delete();
message.channel.send({embed: {
    color: 1585407,
    title: "__**CYBERAM'S RULES**__",
    url: "https://imgur.com/55npvIb",
    description: "These are the CYBERAM rules for all cyber`s existing here",
    fields: [{
        name: "DM-LINKS",
        value: "Don`t dm and advertise with the people in this server.if so found a report then be ready for the BAN"
      },
      {
        name: "ARGUMENTS",
        value: "ARGUMENTS AND DISAGREEMENTS MAY HAPPEN, IF YOU ARE ASKED TO STOP, THEN STOP."
      },
      {
        name: "info-vampire_people",
        value: "VAMPIRES ARE HIDDEN HERE SO, BE CAREFUL , THEY ACT LIKE A IDIOT MAINLY SO..... http://www.skidmore.edu/~pdwyer/e/eoc/help_vampire.htm"
      },
      {
        name: "user-names",
        value: "IF YOUR USERNAME CAN'T BE EASILY MENTIONED, PLEASE SET YOUR NICKNAME TO SOMETHING MENTIONABLE. FOR EXAMPLE,ᐃᑦᑎᓂᓯᐅᑐᑕ {CHANGE IT I SUGGEST}"
      },
      {
        name: "+c.t.f+",
        value: "No too much CAPS+NO TROLLS+HELP PEOPLE IF YOU ARE WITH CORRECT THINGS+NO FORCING"
      },
      {
        name: "staff-status",
        value: "THE STAFF ARE NOT ALLOWED TO BE IN OFFLINE STATUS WHEN ACTUALLY ON"
      },
      {
        name: "alts",
        value: "no spams,no harmful codes/bots"
      },
      {
        name: "channel_topic",
        value: "be sure in which channel you are chatting,also be stick to the channel topic"
      },
      {
        name: "mentions",
        value: "dont mention the people,staff,cyber. without any good reason or any proper work"
      },
      {
        name: "BOT`S COMMANDS",
        value: "Don`t use bot commands out of bots-highway  (except @OC~CYBOT COMMANDS )"
      }
    ],
    timestamp: new Date(),
    footer: {
...
if (util.commandIs(message, "staff-info")){
message.channel.send({embed: {
    color: 3447003,
    author: {
    },
    title: "STAFF OF CYBERAM",
    url: "https://docs.google.com/forms/d/1usQTe4V81gg-jmW2w29IkYWaxva9hsZ6MaWSp-Qfoic/",
    description: "rescepted staff names are given below ",
    fields: [{
        name: "staff1",
        value: "@jonasrog#9081 ,@Terminited#3366 , @Vunh#9888 , @•˜∫Kirdøg~Shade~Slayer∫˜•#6637 , @SailorStar#0354"
      },
      {
        name: "how to be staff",
        value: "i think you should go and check #applcation OR take a shory-cut by just clicking on the top text as `staff of cyberam`"
      }
    ],
    timestamp: new Date(),
    footer: {
...
if (util.commandIs(message, "rps")){
message.reply("1,2,3 here goes the rps")
    let emote = '';
switch(Math.floor((Math.random() * 3) + 1)) {
    case 1:
        emote = ':newspaper: its a paper';
        break;
    case 2:
        emote = ':scissors: its a scissor';
        break;
    case 3:
        emote = ':new_moon: its rock!';
        break;
}
message.reply(emote);
}
if (util.commandIs(message, "mbs")){
...
if (util.commandIs(message, "goi")){
   message.delete();
    message.channel.send({embed: {
        ...
        url: "https://docs.google.com/forms/d/1usQTe4V81gg-jmW2w29IkYWaxva9hsZ6MaWSp-Qfoic/",
        description: "a EVENT WHICH IS FOREVER gonna be there!",
        fields: [{
            name: "premuim bots",
            value: "PER 10 you could have any PREMUIM BOT FROM THE LIST"
          },
          {
            name: "premuim-bots---list",
            value: "invite manager pro , twitch premuim , marv patreon, dank memer beta"
          },
          {
            name: "DANK MEMER BOT coins!",
            value: "get 1k coins per 8invites!"
          }
        ],
        timestamp: new Date(),
        footer: {
          text: "©cyberam`giveaways"
        }
    }

});
}
if(util.commandIs(message, "rrp")){
    message.delete();
    const oyun = [
        `*NOTHING FOUND! KEEP SEARCHING FOR THE Candy!*, `];
       message.author.send(oyun) 
    }
    if(util.commandIs(message, "type")){
        message.delete();
        message.channel.startTyping();
    }
    if(util.commandIs(message, "stop-type")){
message.delete
